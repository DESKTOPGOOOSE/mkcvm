...
case "$packageman" in
    apt) sudo apt install -y make build-essential clang libboost-all-dev libsqlite3-dev libvncserver-dev libpng-dev libturbojpeg-dev libcairo-dev git npm;;
...
echo '[i] 1.2.11 does not have a webserver so you will need to host the webapp and admin panel on another webserver such as nginx.
